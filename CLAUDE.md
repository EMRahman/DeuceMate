...
- **Before pushing any change to a synced setting**, grep the key string across
  ...
  (phone) — the documented announcements aliases (TECHNICAL_DEBT #3).
...
| `Models/PointStat.swift` | `PointOutcome`, `EndingShot`, `GameScoreSnapshot`, `PendingPointInfo`, `PointStat`. The atom from which all stats derive. |
| `Scoring/ScoringEngine.swift` | Pure scoring reducer — `pointWon(by:in:) -> ScoringResult` over value-type `ScoringState`; side-effects come back as typed `ScoringEvent`s. The watch `ScoreViewModel` delegates here. **Change scoring rules here, with `ScoringEngineTests`.** |
...
| `Stats/RecCoachInsights.swift`, `PulseCoachInsights.swift`, `SetActivitySplit.swift`, `HRZone.swift` | Coaching/heart-rate analytics. |
| `WebExport/` (`MatchWebViewModel.swift` + `…+Build.swift` + `…+Comparison.swift` + `…+AICoach.swift`, `MatchHTMLExporter.swift`, `MatchWebStaticFallback.swift`, `MatchWebTemplate.swift`, `WebExportColors.swift`) | Self-contained **interactive HTML match export** — a pure `Encodable` view-model (both me/opponent perspectives flattened, recorder-only HR/steps, + per-set `filters` (All / Set N) each with a recorder-framed TV-style Me-vs-Opp `comparison`, points-won header, and duration rows, + an optional `aiCoach` block, mirroring `MatchDetailView`) + assembler + the viewer (HTML/CSS/dependency-free SVG-JS) kept as Swift **raw-string constants** (no resource bundle, no pbxproj edit). The viewer is recorder-framed — **no perspective toggle** — and mirrors the iOS archive detail: `PointsGraphView`-style scatter pills drive the chart, a **Stats/Points tab toggle** + **All/Set N set filter** switch the lower content, a point-by-point list renders the Points tab, and an **AI Coach card** (mirrors `AICoachSheet`) copies the prompt + offers opt-in AI-app launch links. Derivation stays in tested Swift; the JS only paints. **AI prompt text is injected** (`MatchHTMLExporter.html(for:aiPromptMe:aiPromptOpponent:)`) from `MatchExporter.aiPromptExport` by `MatchDetailView` — the only place outside Core's tested derivation. The page loads zero external resources on open; the AI links are user-clicked navigations (a refined `MatchWebExportTests` invariant — no `src=`/`<link>`/`@import`/CDN). **Progressive enhancement:** `#root` ships pre-filled with a static report from `MatchWebStaticFallback` (header + four server-rendered SVG momentum charts — one per preset: Points Won/Lost + Ending Shots All Won/Lost, each with scatter + colour/label/count pills — + Me/Opp stat tables) so no-JS previews (iOS Quick Look, local-file opens) aren't blank; the viewer JS does `root.innerHTML = ""` and rebuilds. `staticChartSVG` mirrors the JS `buildSVG`/`stepPath`/`symbol` geometry — keep them in step. HR is recorder-only (mirrors `MatchExporter`). Bump `MatchWebViewModel.currentSchemaVersion` when the JSON shape changes. `MatchDetailView` shares the produced `.html` as a file. Keep `WebExportColors` in step with `PointsGraphView`'s palette. See `docs/features/INTERACTIVE_HTML_EXPORT_PLAN.md`. |
...
| File | Lines | What |
|------|------:|------|
| `ScoreViewModel.swift` | ~1928 | Live match state + synced settings; delegates scoring rules to Core's `ScoringEngine` (see §1). ⚠️ Almost no `MARK:` anchors yet — `Grep` for the symbol and read a bounded range. |
| `HomeView.swift` | ~900 | Match setup / start screen. ⚠️ No `MARK:` anchors yet. |
...
| File | Lines | What |
|------|------:|------|
| `Views/PointsGraphView.swift` | ~1525 | Charts (points momentum + HR/steps overlays). Heavily `MARK:`-sectioned. |
...
- **Very large files** (`ScoreViewModel` ~1.9k, `PointsGraphView` ~1.5k,
  `MatchDetailView` ~1.0k, `ContentView`/`HomeView`/`PastMatchesView`
