...
| `Stats/MatchTrendSample.swift`, `TrendMetric.swift`, `PerformanceTrends.swift` | Cross-match performance trends (the archive-screen Trends feature). `MatchTrendSample.init?(record:)` reduces one eligible match to recorder-framed raw counters via a single `MatchStatsSummary(focal: .me)` — never a second counting pass, and never `opponentForcedErrors` read backwards as "forced errors I was forced into" (it means the reverse). `TrendMetric` is the data-driven metric catalogue, with `rawPair(in:)` (for pooling, unguarded) split from `ratio(in:)` (for plotting one dot, denominator-guarded) so a flawless zero-error match's winners aren't discarded when pooled. `PerformanceTrends` filters **before** windowing, pools as Σnum/Σden (never a mean of per-match rates), and orients every delta by the metric's `betterDirection` so a falling double-fault rate is always `.improving`. The catalogue also carries 13 **health** metrics in three groups (Heart Rate, Movement, Fatigue) and the four non-percent `Unit` cases they need (`.bpm`/`.steps`/`.metres`/`.minutes`) — a metric's unit **is** the axis it belongs on, which is what `TrendChart` buckets by. Their counters are projections of the SAME summary's `hrTimeline`/`stepsTimeline`/`zoneWinRates` (never a second pass); every one `rawPair`-gates to `nil` below its coverage threshold so a Health-free match leaves a gap, not a zero; the steps figures come from the sampled timeline and **never** `StepsSeries`' `totalSteps` fallback (which would make steps-per-point a constant by construction); and `MatchTrendSample.fatigue` resolves the first vs last *played* set once per match (20 points each, `nil` while in progress). `init?(record:maxHR:)` takes the zone yardstick — a live setting applied retroactively, so cachers must key on it. See `docs/features/PERFORMANCE_TRENDS_PLAN.md` and `docs/features/HEALTH_TRENDS_PLAN.md`. |
