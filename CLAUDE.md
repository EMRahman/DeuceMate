...
| `WebExport/` (`MatchWebViewModel.swift` + `…+Build.swift`, `MatchHTMLExporter.swift`, `MatchWebTemplate.swift`, `WebExportColors.swift`) | Self-contained **interactive HTML match export** — a pure `Encodable` view-model (both me/opponent perspectives flattened, recorder-only HR/steps) + assembler + the viewer (HTML/CSS/dependency-free SVG-JS) kept as Swift **raw-string constants** (no resource bundle, no pbxproj edit). Derivation stays in tested Swift; the JS only paints. HR is recorder-only (mirrors `MatchExporter`). `MatchDetailView` shares the produced `.html` as a file. Keep `WebExportColors` in step with `PointsGraphView`'s palette. See `docs/features/INTERACTIVE_HTML_EXPORT_PLAN.md`. |
