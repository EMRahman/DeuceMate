...
| `Settings/MatchTrackingStatus.swift` | The pre-match "what will this match record?" rule: `pointTracking` / `healthTracking` / `pulseCoach` resolved to `.on` / `.partial` / `.off` plus label, SF Symbol, and fix-it copy. `HealthAccess` is the portable mirror of the HealthKit permission (the watch maps `HKHealthStore` onto it — Core stays HealthKit-free). Drives the watch start-screen tracking strip and its settings summary. |
...
| `HomeView.swift` | ~915 | Match setup / start screen (scrolling), incl. the tracking-status strip under Start Match and the settings sheet. ⚠️ No `MARK:` anchors yet. |
| `TrackingStatusStrip.swift` | ~130 | Paints Core's `MatchTrackingStatus`: the three-chip Points / Health / Pulse strip on the start screen (taps to Settings) and the same three as rows at the top of the settings sheet. Observes `WorkoutManager` directly — `ScoreViewModel` does not republish Health access. |
