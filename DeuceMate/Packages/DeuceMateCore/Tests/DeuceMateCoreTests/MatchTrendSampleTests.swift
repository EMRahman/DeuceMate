...
final class MatchTrendSampleTests: XCTestCase {
    ...
    func test_counters_matchHandBuiltExpectations() {
        ...
        // secondServePoints: 2 (pts server=me, isSecondServe=true — one .winner, one .doubleFault)
        // secondServesIn: 1 (only the .winner one landed in; the .doubleFault one did not)
        XCTAssertEqual(s.secondServePoints, 2)
        XCTAssertEqual(s.secondServesIn, 1)
