...
final class PersistenceHealthTests: XCTestCase {
    ...
    func test_operationsThatCostMatchDataAreCritical() {
        // Critical means "something the user produced is not durable". A failed
        // live-state restore qualifies: the match is out of the app's view and
        // the file holding it is one write away from being overwritten.
        XCTAssertEqual(PersistenceOperation.saveLiveMatch.severity, .critical)
        XCTAssertEqual(PersistenceOperation.restoreLiveMatch.severity, .critical)
        ...
        // These leave the stored data intact and only degrade what is shown.
    ...
    func test_repeatOfTheSameFailure_reportsNoVisibleChange() {
        var health = PersistenceHealth()
        XCTAssertTrue(health.record(failure(.saveLiveMatch, "first")))

        // The live checkpoint is written on every point, so a persistent write
        // failure repeats indefinitely. The copy is identical, so republishing
        // would redraw the scoreboard per point for an unchanged banner.
        let before = health.warning
        XCTAssertFalse(health.record(failure(.saveLiveMatch, "second")))
        XCTAssertEqual(health.warning, before)

        // The newest failure is still the one held — it describes the store now.
        XCTAssertEqual(health.failure?.detail, "second")
    }

    func test_repeatViaApply_alsoReportsNoVisibleChange() {
        var health = PersistenceHealth()
        XCTAssertTrue(health.apply(.failed(failure(.readMatchHistory))))
        XCTAssertFalse(health.apply(.failed(failure(.readMatchHistory))))
    }
}
