...
public enum PersistenceOperation: String, CaseIterable, Sendable {
    ...
    public var severity: PersistenceSeverity {
        switch self {
        case .saveLiveMatch, .saveMatchHistory, .archiveWritesSuspended:
            return .critical
        case .restoreLiveMatch, .readMatchHistory, .archiveQuarantined:
            return .warning
        }
    }
}
...
public struct PersistenceHealth: Equatable, Sendable {
    ...
    /// Record a failure. Returns `true` when it became the displayed failure,
    /// so callers can skip publishing an unchanged value.
    @discardableResult
    public mutating func record(_ incoming: PersistenceFailure) -> Bool {
        if let current = failure, current.operation.severity > incoming.operation.severity {
            return false
        }
        failure = incoming
        return true
    ...
    public static func warning(for operation: PersistenceOperation) -> PersistenceWarning {
        ...
        case .restoreLiveMatch:
            return PersistenceWarning(
                ...
                message: "A saved match was found but couldn't be read, so scoring started fresh.",
