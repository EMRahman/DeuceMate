...
public struct MatchTrackingStatus: Sendable, Equatable {
    ...
    /// One line explaining the current state — and, when it is not `.on`, how to fix it.
    ...
    /// Spoken/VoiceOver form, e.g. "Point tracking: Off. …".
    public var accessibilityDescription: String { "\(title): \(stateLabel). \(detail)" }
    ...
    /// Point tracking is a plain user toggle — it is on or it is off.
    public static func pointTracking(isEnabled: Bool) -> MatchTrackingStatus {
        MatchTrackingStatus(
            facet: .pointTracking,
    ...
    public static func pulseCoach(
        ...
        guard hasOverride || hasBirthYear else {
            return MatchTrackingStatus(
                ...
                readiness: .partial,
                stateLabel: "Est.",
                detail: SettingsCopy.defaultMaxHRNote
            )
        }
    ...
    /// future iPhone mirror can never show a different set or a different order.
    public static func all(
        ...
        [
            pointTracking(isEnabled: pointTrackingEnabled),
