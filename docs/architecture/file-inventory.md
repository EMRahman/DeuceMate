...
| `Stats/PointGamesScore.swift` | 60 | Derives the running Me–Opponent games score within a set at the start of each point, purely from `PointStat.gameScoreAtStart` resets (no stored field carries it), reconciled against `MatchRecord.setScores` so a set whose tracked points are only a suffix (e.g. a `ManualMatchEntryView` mid-set reconstruction resumed on the watch) is suppressed rather than mislabeled. Empty for deciding super-tiebreak sets and tiebreak-only/perpetual match formats too. Shared by the iOS Points tab and the HTML export's Points tab. | Stats, Web export |
