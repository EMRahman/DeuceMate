...
| `PersistenceHealthTests.swift` | The persistence-failure precedence rules: severity ordering, a warning never displacing a critical failure, newest-wins at equal severity, per-operation success clearing, acknowledgement, and distinct non-empty user-facing copy for every operation. |
