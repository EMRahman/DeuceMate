...
write can't clear a live-match save failure). `PersistenceWarning` carries the
user-facing copy, so the watch and phone banners can't drift.
...
rendered as a dismissible banner above Start/Resume Match; `PhoneStatsStore`
publishes the same type for failed canonical writes, writes suspended after an
...
**Tests added:** `PersistenceHealthTests` (Core, precedence + copy), plus
reporting/health cases in `StatsStoreTests` and `PhoneStatsStoreTests`.
